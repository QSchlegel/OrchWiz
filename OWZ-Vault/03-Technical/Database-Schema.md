...
### AgentSyncPreference
- Per-user AgentSync settings
- Stores timezone, nightly enabled toggle, nightly hour, and last nightly run timestamp

### AgentSyncSignal
- Deduped reinforcement signal events per source/subagent/user
- Sources: `command`, `verification`, `bridge_call`
- Unique key: `(source, sourceId, subagentId)`

### AgentSyncRun
- Manual/nightly AgentSync execution record
- Stores trigger (`manual|nightly`), scope (`selected_agent|bridge_crew`), run status, summary, metadata, and file-sync state

### AgentSyncSuggestion
- Proposed/applied/rejected managed context updates generated by AgentSync
- Tracks file risk classification (`low|high`), suggestion status, suggested content, apply timestamps, and file-sync state
