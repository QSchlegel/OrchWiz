...
function proxyBasePath(): string {
  return "/api/bridge/runtime-ui/prometheus"
}
...
function resolvePrometheusUpstreamBaseUrl(): string {
  const override = asString(process.env.PROMETHEUS_UPSTREAM_URL)
  if (override) {
    try {
  ...
  if (runningInKubernetes) {
    return `http://prometheus-server.${monitoringNamespace}.svc.cluster.local:80`
  }

  // Outside Kubernetes, route through runtime-edge so users only need one port-forward.
  return "http://127.0.0.1:3100/prometheus"
}
  ...
  try {
    upstream = await fetch(upstreamUrl, {
      ...
      headers: {
        ...
        "User-Agent": request.headers.get("user-agent") || "OrchWiz-Bridge-PrometheusProxy",
      },
      redirect: "manual",
    })
  } catch (error) {
...
export async function GET(
  ...
  try {
    ...
    console.error("Bridge Prometheus runtime UI proxy failed:", error)
    return NextResponse.json(
