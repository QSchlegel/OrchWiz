...
Quartermaster fallback diagnostics:

- Quartermaster fallback responses now store structured diagnostics under interaction metadata:
  - `fallback.active`
  - `fallback.provider`
  - `fallback.reason`
  - `fallback.providerErrors[]` (`provider`, `code`, `message`)
- These diagnostics are intended for UI details panels and troubleshooting without exposing raw runtime prompt text.
