#!/usr/bin/env python3
"""Install a tool from a GitHub repo path into $CODEX_HOME/tools."""
...
# Reuse existing authenticated GitHub helper from skill installer scripts.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SKILL_INSTALLER_DIR = os.path.normpath(os.path.join(SCRIPT_DIR, "..", "skill-installer"))
if SKILL_INSTALLER_DIR not in sys.path:
    sys.path.insert(0, SKILL_INSTALLER_DIR)

from github_utils import github_request  # type: ignore
...
def _request(url: str) -> bytes:
    return github_request(url, "codex-tool-install")
...
def _validate_relative_path(path: str) -> None:
    if os.path.isabs(path) or os.path.normpath(path).startswith(".."):
        raise InstallError("Tool path must be a relative path inside the repo.")


def _validate_tool_name(name: str) -> None:
    altsep = os.path.altsep
    if not name or os.path.sep in name or (altsep and altsep in name):
        raise InstallError("Tool name must be a single path segment.")
    if name in (".", ".."):
        raise InstallError("Invalid tool name.")


def _validate_tool(path: str) -> None:
    if not os.path.isdir(path):
        raise InstallError(f"Tool path not found: {path}")

    entries = os.listdir(path)
    if not entries:
        raise InstallError("Selected tool directory is empty.")


def _copy_tool(src: str, dest_dir: str) -> None:
...
def _default_dest() -> str:
    return os.path.join(_codex_home(), "tools")


def _parse_args(argv: list[str]) -> Args:
    parser = argparse.ArgumentParser(description="Install a tool from GitHub.")
    parser.add_argument("--repo", help="owner/repo")
    parser.add_argument("--url", help="https://github.com/owner/repo[/tree/ref/path]")
    parser.add_argument("--path", nargs="+", help="Path(s) to tool(s) inside repo")
    parser.add_argument("--ref", default=DEFAULT_REF)
    parser.add_argument("--dest", help="Destination tools directory")
    parser.add_argument("--name", help="Destination tool name (defaults to basename of path or repo)")
...
def _derive_name(path: str, fallback_repo: str | None) -> str:
    basename = os.path.basename(path.rstrip("/"))
    if basename and basename not in (".", ".."):
        return basename
    if fallback_repo:
        return fallback_repo.split("/")[-1]
    return "tool"
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if args.name:
            _validate_tool_name(args.name)

        dest_root = os.path.abspath(args.dest or _default_dest())
        os.makedirs(dest_root, exist_ok=True)

        with tempfile.TemporaryDirectory(prefix="codex-tool-", dir=_tmp_root()) as tmp_dir:
            repo_root = _prepare_repo(source, args.method, tmp_dir)

            # If no explicit path is provided, install repository root as one tool.
            paths = source.paths or [""]
            for path in paths:
                src_path = os.path.join(repo_root, path) if path else repo_root
                src_path = os.path.normpath(src_path)
                _validate_tool(src_path)

                tool_name = args.name if args.name else _derive_name(path, f"{source.owner}/{source.repo}")
                _validate_tool_name(tool_name)

                destination = os.path.join(dest_root, tool_name)
                _copy_tool(src_path, destination)
                print(f"Installed {tool_name} to {destination}")
