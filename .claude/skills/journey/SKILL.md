...
Use `/journey` for a quick standalone capture. For end-of-session automation that also evolves skills, use `/wrap-up` instead.
...
- `$0` - Journey title describing the main work done
...
After a session refactoring API authentication:

```
/journey API Auth Refactor
```

Claude will create `.claude/journeys/2026-02-08-api-auth-refactor.md` with:
- Summary of what was refactored
- Key learnings about the auth flow
- Files that were changed
- Patterns discovered (like token refresh handling)
