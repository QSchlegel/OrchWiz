...
function setupHints(args: {
  ...
  if (!args.binaryAvailable) {
    return [
      "Install Codex CLI and verify the binary is on PATH.",
      "Set CODEX_CLI_PATH if you need an explicit binary path.",
    ]
  }

  if (args.accountConnected) {
    return [
      "Codex CLI is connected and ready for provider-proxy requests.",
    ]
  }
